...
local function commandLog(cmd)
	...
	if not (strutils.checkNil(cmd)) then
		...
		if (strutils.checkStringEquals(cmd, "create-account")) then console_writefile("Entered account creation.", logfile) end
...
function parser.parseCommand(cmd)
	...
	local console_displayer = require("src.modules.display.console_displayer")
	local u_sign = require("src.modules.gui.user_signup")
	...
	if not (strutils.checkNil(cmd)) then
		...
		if (strutils.checkStringEquals(cmd, "exit")) then
			sys.exitProgram()
			sys.clearScreen()
			command_properties.do_init = false
		end
		...
		if (strutils.checkStringEquals(cmd, "create-account")) then
			u_sign.init()
		end
